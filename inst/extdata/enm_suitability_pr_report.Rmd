...
This model was projected to `r length(scenarios)` scenario(s). The results for all projections can be found in the output directory defined in the function `ellipsoid_model` under the name(s) (`r paste(scenarios, collapse = ", ")`). Graphical and numerical results are presented only for the `r as.character(scenarios[1])` scenario.
...
```{r Figure2, fig.cap="Representation of species occurrence data in environmental space for the calibration area. Grey points respresent environmental conditios in the calibration area; blue points respresent occurrence data."}
if (n_var > 2) {
...
```{r Table2}
kable(prevalences, row.names = TRUE, caption = paste0("Prevalence of the ellipsoid envelope model for ", as.character(data[1, 1]), " in the area of calibration."))
```
...
```{r Figure3, fig.cap="Representation of suitability values in environmental space for the calibration area. Occurrence data is represented in blue. Cold colors represent low suitabilities and warm colors high suitabilities."}
...
A `r paste(ifelse(n_var > 2, "3", "2"), "dimensional")` representation of the ellipsoidal niche model and the suitability values is shown in environmental space for a scenario `r as.character(scenarios[1])`.

```{r Figure4, fig.cap="Representation of suitability values in environmental space for a scenario of projection. Occurrence data is represented in blue. Cold colors represent low suitabilities and warm colors high suitabilities."}
...
```{r Figure5, fig.cap="Representation of suitability values in geographic space. Occurrence data is represented in blue. Cold colors represent low suitabilities and warm colors high suitabilities. Grey lines represent country borders."}
par(mar = c(2, 2, 1, 1))
plot(layer, col = color_palette(255))
map(add = TRUE, col = "gray50")
points(data[, 2:3], col = "blue3")
```
...
```{r Figure6, fig.cap="Representation of suitability values in geography for a projection scenario. Occurrence data is represented in blue. Cold colors represent low suitabilities and warm colors high suitabilities. Grey lines represent country borders."}
par(mar = c(2, 2, 1, 1))
plot(layer_projection, col = color_palette(255))
map(add = TRUE, col = "gray50")
points(data[, 2:3], col = "blue3")
```
