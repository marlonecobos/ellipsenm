...
The following chunk of code helps to visualize the accessible area to be used in further explorations.

```{r loading_M}

```

If the user does not have a shapefile or raster of a hypothesis of **M** you can create explore different options. Three potential options can be used to define the **M**: a manual definition of accessible areas for the species, the use of geographic polygons (e.g., buffers, convex hulls, or concave hulls). A manual definition of the M can be done using distinct software, then it can be exported and 
