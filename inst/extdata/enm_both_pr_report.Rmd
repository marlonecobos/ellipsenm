...
This model was projected to `r length(scenarios)` scenario(s). The results for all projections can be found in the output directory defined in the function `ellipsoid_model` under the name(s): `r paste0(paste(scenarios[-length(scenarios)], collapse = ", "), ", and ", scenarios[length(scenarios)])`. Graphical and numerical results are presented only for the `r as.character(scenarios[1])` scenario.
