...
Prevalence for the `r as.character(scenarios[1])` scenario are presented below (if more than one projection was performed, prevalence in other scenarios can be found in the output directory).
...
```{r Figure4, fig.cap="Representation of suitability values in environmental space. Occurrence data is represented in blue. Cold colors represent low suitabilities and warm colors high suitabilities."}
...
```{r Figure5, fig.cap="Representation of Mahalanobis distances in environmental space. Occurrence data is represented in blue. Cold colors represent long distances and warm colors short distances."}
...
Following, a `r paste(ifelse(n_var > 2, "3", "2"), "dimensional")` representation of the ellipsoidal niche model and the suitability values is shown in environmental space for the `r as.character(scenarios[1])` scenario.

```{r Figure6, fig.cap="Representation of suitability values in environmental space for a scenario of projection. Occurrence data is represented in blue. Cold colors represent low suitabilities and warm colors high suitabilities."}
col1 <- color_palette(nrow(pr_values))

if (n_var > 2) {
  suit <- predict(ob, pr_values[, 1:3], "suitability", TRUE)
  
  vals <- cbind(pr_values[, 1:3], suit@suitability)
  vals <- vals[order(vals[, 4], decreasing = F), ]
  vals1 <- vals[vals[, 4] > 0, -4]
  vals0 <- vals[vals[, 4] == 0, -4]
  ...
  suit <- predict(ob, pr_values[, 1:2], "suitability", TRUE)
  
  vals <- cbind(pr_values[, 1:2], suit@suitability)
  vals <- vals[order(vals[, 3], decreasing = F), ]
  vals1 <- vals[vals[, 3] > 0, -3]
  vals0 <- vals[vals[, 3] == 0, -3]
...
A `r paste(ifelse(n_var > 2, "3", "2"), "dimensional")` representation of the ellipsoidal niche model and the Mahalanobis distances for the `r as.character(scenarios[1])` scenario is shown in the following figure.

```{r Figure7, fig.cap="Representation of Mahalanobis distances in environmental space. Occurrence data is represented in blue. Cold colors represent long distances and warm colors short distances."}
...
```{r Figure8, fig.cap="Representation of suitability values in geographic space. Cold colors represent low suitabilities and warm colors high suitabilities. Grey lines represent country borders."}
par(mar = c(2, 2, 1, 1))
...
```{r Figure9, fig.cap="Representation of Mahalanobis distances in geographic space. Cold colors represent long distances and warm colors short distances. Grey lines represent country borders."}
par(mar = c(2, 2, 1, 1))
...
```{r Figure10, fig.cap="Representation of suitability values in geography for a projection scenario. Cold colors represent low suitabilities and warm colors high suitabilities. Grey lines represent country borders."}
par(mar = c(2, 2, 1, 1))
plot(layer_projection[[1]], col = color_palette(255))
map(add = TRUE, col = "gray50")
```

A geographic prediction of Mahalanobis distances fo the `r as.character(scenarios[1])` scenario is shown below. 

```{r Figure11, fig.cap="Representation of Mahalanobis distances in geography for a projection scenario. Cold colors represent long distances and warm colors short distances. Grey lines represent country borders."}
par(mar = c(2, 2, 1, 1))
plot(layer_projection[[2]], col = rev(color_palette(255)))
map(add = TRUE, col = "gray50")
```
...
The code for producing this report (if needed) can be found in the file `enm_both_pr_report.Rmd`; the data used is in `enm_report_data.RData`.
