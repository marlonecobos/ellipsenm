...
Note 3: This is the report of a projected model, only one example of the projected results is shown graphically to avoid RAM-related issues. If projections were made to more than one scenario, other results to replicate figures presented here can be found in the output directory defined in the function `ellipsoid_model`.  
...
Prevalence in environmental and geographic space are summarized below for all areas and scenarios of projection.
...
```{r Figure3, fig.cap="Representation of suitability values in environmental space for the calibration area. Cold colors represent low suitabilities and warm colors high suitabilities."}
...
A `r paste(ifelse(n_var > 2, "3", "2"), "dimensional")` representation of the ellipsoidal niche model and the suitability values is shown in environmental space for a scenario of projection.

```{r Figure4, fig.cap="Representation of suitability values in environmental space for a scenario of projection. Cold colors represent low suitabilities and warm colors high suitabilities."}
...
if (n_var > 2) {
  ...
  plot3d(data[, 4:6], col = "black", size = 5)
  plot3d(pr_values[order(suit@suitability, decreasing = F), 1:3], col = col1, add = TRUE)
  wire3d(el3d, col = "darkgreen", alpha = 0.5)
  rglwidget()
  ...
  par(mar = c(4, 4, 1, 1))
  plot(pr_values[order(suit@suitability, decreasing = F), 1:2], col = col1)
  points(data[, 4:5], pch = 19) 
  lines(el1, col = "darkgreen", lwd = 1.5)
}
```
...
```{r Figure5, fig.cap="Representation of suitability values in geography for the calibration area."}
par(mar = c(2, 2, 1, 1))
plot(suit_layer, col = color_palette(255))
map(add = TRUE, col = "gray50")
```

## Projection area

A geographic prediction in a projection scenario is shown below. This example corresponds to the scenario `r sceario_name`.

```{r Figure6, fig.cap="Representation of suitability values in geography for a projection scenario."}
par(mar = c(2, 2, 1, 1))
plot(proj_layer, col = color_palette(255))
map(add = TRUE, col = "gray50")
```
...
The code for producing this report (if needed) can be found in the file `enm_suitability_projected_report.Rmd`; the data used is in `enm_report_data.RData`.
