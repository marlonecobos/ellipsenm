...
suppressWarnings(library(oompaBase))
...
Numeic results of the ellipsoid envelope model for *`r as.character(data[1, 1])`* are presented below. 
...
Prevalence in environmental and georaphic space are sumarized below.
...
Given the data and the characteristics that define the species ellipsoidal niche (see [Data used for modeling] and [Ellipsoid characteristics]) the results in geographic space are presented bellow. Values of suitability can range from 0 to 1; when the maximum is not 1, it means that there is not environmental conditions equal to the ones at the centroid of the ellipsoid. Another reason why the range of suitability can change is that a different function of suitability was used in the `ellipsoid_model` function. Values of cero (cold areas) represent areas with environmental values outside the ellipsoid, values other than cero reprensent levels of suitability. 

## Calibration area

The geographic prediction in the calibration area is presented below. 

```{r Figure5, fig.cap="Representation of suitability values in geographic space."}
par(mar = c(2, 2, 1, 1))
plot(suit_layer, col = jetColors(255))
```

<hr>

This report was produced usign the <a href="https://github.com/marlonecobos/ellipsenm" target="_blank">**`ellipsenm`**</a> R package; to cite R and this package use the following code:
