...
#'
#' @description predict_rep predicts suitability values based on mahalanobis
...
predict_rep <- function(object, projection_layers, prediction = "suitability",
  ...
  if (prediction != "mahalanobis") {
    ...
    if (prediction == "both") {
      ...
      if (return_numeric == TRUE) {
        ...
        results <- ellipsoid_model_rep(method = object@method,
                                       level = object@level,
